...
appRoot.innerHTML = /* html */ `
  <div class="flex flex-col h-screen bg-base-100 text-base-content">
    ...
    <div class="flex items-center gap-2 p-2 border-b border-base-300">
      ...
      <select id="range" class="select select-bordered select-xs mail-only" title="Show mail received within">
    ...
    <div class="settings-panel shortcuts-panel" role="dialog" aria-modal="true" aria-label="Keyboard shortcuts">
      ...
      <table class="shortcuts-table">
        <tbody>
          ...
          <tr><td><kbd>Enter</kbd></td><td>Open message in a new window</td></tr>
...
const rangeSelect = document.querySelector<HTMLSelectElement>("#range")!;
...
// Open the row under the cursor: drafts resume editing, anything else opens in
// its own window (Enter, like a double-click — Outlook parity).
function activateCursor(): void {
  if (!cursorId) return;
  if (!searchActive && currentFolderIsDrafts()) void resumeDraft(cursorId);
  else openMessageWindow(cursorId);
}

// Open a message in a standalone OS window (double-click / Enter / context menu
// "Open"). Single-click still previews in the reading pane. Re-opening the same
// message focuses its existing window (handled in the backend). `id` alone is
// enough; the subject for the window title is read from the row when present.
function openMessageWindow(id: string, subject?: string): void {
  const subj = subject ?? rowFor(id)?.querySelector<HTMLElement>(".msg-subject")?.title ?? "";
  void invoke("open_message_window", { messageId: id, subject: subj }).catch((e) => {
    statusEl.textContent = `Failed to open message window: ${e}`;
  });
}
...
// Broadcast that a message left its folder so any open pop-out window for it
// closes itself.
function notifyMessageRemoved(id: string): void {
  void emit("message-removed", { messageId: id });
}
...
// Adaptive dark-mode email rendering (colour probing, contrast adaptation, iframe
// wrapper) lives in ./email-render.ts, shared with the pop-out message window.
...
// Returns false when the server call failed (offline); true otherwise.
async function backfillOlder(): Promise<boolean> {
  const fid = currentFolderId;
  if (!fid || backfilling) return false;
  backfilling = true;
  ...
  try {
    ...
    if (fid !== currentFolderId || searchActive) return true; // folder switched / search started
...
// Double-click opens the message in its own window (single-click still previews
// in the reading pane). Modifier-clicks are selection, not open.
listEl.addEventListener("dblclick", (e) => {
  if (e.ctrlKey || e.metaKey || e.shiftKey) return;
  const row = (e.target as HTMLElement).closest<HTMLElement>(".msg");
  if (!row?.dataset.id) return;
  const id = row.dataset.id;
  // Drafts keep their resume-in-compose behaviour (no pop-out compose window).
  if (!searchActive && currentFolderIsDrafts()) return;
...
function renderMainMenu(unread: boolean, flagged: boolean): void {
  const items: Array<{ act: string; label: string; danger?: boolean } | "sep"> = [
    { act: "open", label: "Open in new window" },
...
// ---- Pop-out message windows ----
// A pop-out window delegates Reply / Forward / Delete / Headers here (it has no
// compose modal or headers overlay of its own). Bring the main window forward
// so the resulting compose / confirmation / overlay is visible.
interface MessageWindowAction {
  messageId: string;
  action: "reply" | "replyAll" | "forward" | "delete" | "headers";
  subject?: string;
}
void listen<MessageWindowAction>("message-window-action", (e) => {
  const { messageId: id, action, subject } = e.payload;
  ...
  switch (action) {
    ...
    case "delete":
      ...
      break;
    case "headers":
      void openHeaders(id, subject ?? "");
      break;
  }
});
// A pop-out marks its message read on open; mirror that in the list so the row
// and unread badges update immediately instead of at the next sync.
void listen<{ messageId: string; read: boolean }>("message-read-changed", (e) => {
  const { messageId: id, read } = e.payload;
  if (!read) return;
  const row = rowFor(id);
